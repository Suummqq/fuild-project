...
#             sf[i,j] = 1         #设置固体区域为1
@ti.kernel
def set_boundary_velocity(vf: ti.template(), sf: ti.template(),x:float,y:float):
    # print(x,y)
    for i,j in vf:
        if ((i + 0.5) - res * x) ** 2 + ((j + 0.5) - res * y) ** 2 <= radius ** 2:
            sf[i,j] = 1
        else:
            sf[i,j] = 0
        if i== 0 :
...
@ti.kernel
def apply_impulse(vf: ti.template(),imp_data: ti.types.ndarray(),sf:ti.template()):
    for i, j in vf:
        # omx, omy = imp_data[2], imp_data[3]             #鼠标点击的坐标位置
        v1 = imp_data[0]/dt             #水平速度
        u1 = imp_data[1]/dt             #垂直速度
        if 0<i<res-1 and 0<j<res-1:
            if (sf[i, j] == 0 and sf[i + 1, j] != 0):  # 碰到墙壁给个反方向的速度
                vf[i, j] -= v1
            if (sf[i, j] == 0 and sf[i-1, j] != 0):  # 碰到墙壁给个反方向的速度
                vf[i, j] += v1
            if (sf[i, j] == 0 and sf[i, j+1] != 0):  # 碰到墙壁给个反方向的速度
                vf[i, j] += u1
            if (sf[i, j] == 0 and sf[i, j-1] != 0):  # 碰到墙壁给个反方向的速度
                vf[i, j] += u1
...
def setup(mouse_data):
    ...
    #根据拖拽的方向更新速度
    # apply_impulse(velocities_pair.cur, mouse_data,solid_fild)               #根据外力更新速度
...
class MouseDataGen:
    def __init__(self):
        self.prev_mouse = center
        # print(center)

    def __call__(self, gui):
        # [0:2]: normalized delta direction 归一化后的鼠标方向
        # [2:4]: current mouse xy   当前鼠标位置
        mouse_data = np.zeros(4, dtype=np.float32)
        global center
        if gui.is_pressed(ti.GUI.LMB):
            xy = np.array(gui.get_cursor_pos(), dtype=np.float32)
            mxy = xy * res
            ...
            self.prev_mouse = mxy
            center = [xy[0], xy[1]]
            # print(mouse_data[0],mouse_data[1])
        return mouse_data
    ...
    while gui.running:
        if gui.get_event(ti.GUI.PRESS):
            ...
            if e.key == ti.GUI.ESCAPE:
                break
        #拖动鼠标
        mousedata = md_gen(gui)
        set_boundary_velocity(velocities_pair.cur,solid_fild,center[0],center[1])
        setup(mousedata)
